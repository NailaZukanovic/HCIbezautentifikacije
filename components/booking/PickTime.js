...
export default (PickTime = ({ navigation, route }) => {
	...
	const { color1, color2, color3, color4,color5,color6,color7,color8 } = colors;
	...
	return (
		<View style={styles.container}>
			<Text style={styles.title}>Odaberi vreme</Text>
			...
			<View style={styles.dayTime}>
				<Button
					onPress={() => setColors({ color4: 'green', color1: 'grey', color2: 'grey',color3: 'grey', color8: 'grey' })}
					title={TimeRange.dayTime.time5}
					color={color4 ? color4 : color}
					accessibilityLabel="Learn more about this purple button"
					style={styles.dayTime1}
				/>
				<Button
			...
			<View style={styles.nextButton}>
				...
				<TouchableOpacity style={styles.iconBtn} onPress={() => navigation.navigate('BookList')}>
					...
					<Text style={styles.next}>Sačuvaj</Text>
				</TouchableOpacity>
			</View>
		</View>
	);
